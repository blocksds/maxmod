...
static IWRAM_CODE ARM_CODE mm_byte mmChannelStartACHN(mm_module_channel* module_channel, mm_active_channel* active_channel, mpl_layer_information* mpp_layer, mm_byte channel_counter) {
    // Clear tremor/cutvol
...
static IWRAM_CODE ARM_CODE mm_bool is_effect_glissando(mm_module_channel* module_channel) {
    return (module_channel->flags & MF_HASFX) && (module_channel->effect == GLISSANDO_EFFECT);
}

static IWRAM_CODE ARM_CODE mm_bool is_vcmd_glissando(mm_module_channel* module_channel, mm_bool is_xm_mode) {
